...
const [orderstatus,setorderstatus]=useState(["approved", "pending", "rejected"]);
 const [agent_withdraw,set_agent_withdraw]=useState([])
const [status,setstatus]=useState(1);
const agent_data_fetch = () => {
    axios.get(`${process.env.REACT_APP_BASE_URL2}/agent-all-withdraw`)
        .then((res) => {
          console.log(res.data)
            set_agent_withdraw(res.data);
        })
        .catch((err) => {
            console.log(err);
        });
         ...
         try {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              axios.put(`${process.env.REACT_APP_BASE_URL2}/agent-withdraw-status/${id}`, {
                status: status_val,
...
const totalPages = Math.ceil(agent_withdraw.length / itemsPerPage);

const indexOfLastItem = currentPage * itemsPerPage;
const indexOfFirstItem = indexOfLastItem - itemsPerPage;
const currentItems = agent_withdraw.slice(indexOfFirstItem, indexOfLastItem);
...
>
  ...
  >
    ...
    {/* Main Content */}
    <section className="w-full p-[10px] lg:p-[20px]">
               <div className="flex justify-between items-center">
               <div>
                 <h1 className="text-[17px] g:text-[22px] font-[600] lg:mb-[5px]">Agent Withdrwals Transaction</h1> 
     ...
     <tr>
                <td className="px-4 py-4 text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">
                  <div className="inline-flex items-center gap-x-3">
                    <input type="checkbox" className="text-blue-500 border-gray-300 rounded dark:bg-gray-900 dark:ring-offset-gray-900 dark:border-gray-700" />
                   <NavLink to={`/agent-deposit-invoice/${data._id}`} className="text-orange-500 hover:underline hover:text-indigo-600"> <span>#{data.invoiceId}</span></NavLink>
                  </div>
                </td>
              ...
              >
                ...
                <td>
             {
              data.status=="approved" ? <span className="px-[10px] py-[6px] rounded-full bg-orange-200 text-orange-600">Approved</span>:<select name="" id="" defaultValue={data.status} onChange={(e)=>{handlestatus(data._id,e.target.value)}} className="px-[20px] py-[10px] border-none appearance-none outline-none  w-auto bg-[#5D87FF] text-white rounded-[5px]">
                      {
